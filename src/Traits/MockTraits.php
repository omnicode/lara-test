...
trait MockTraits
{
     ...
     * @return mixed
     */
    protected function chainMethodsWillTrue($object, $methods)
    {
        $method = head(array_keys($methods));
        $with = array_shift($methods);
        $with = (array) $with;

        if (empty($methods)) {
            return $this->fixExpectsMethod($object, $method, 'once', '', $with)->willReturn(true);
        }

        $mockObject = $this->getMockObjectWithMockedMethods(head(array_keys($methods)));
        $this->chainMethodsWillTrue($mockObject, $methods);
        return $this->fixExpectsMethod($object, $method, 'once', '', $with)->willReturn($mockObject);
    }
